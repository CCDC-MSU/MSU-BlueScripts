...
GITHUB_URL="https://github.com/CCDC-MSU/MSU-BlueScripts/tree/main/"
...
# Maximum login attempts to prevent infinite loops
...
function autodetect_os {
    ...
    if [ $pacman == 0 ]; then
        info "pacman detected (Arch-based OS)"
        info "Arch Linux uses systemd-journald and pacman for package management"
        PM="pacman"
    elif [ $apt == 0 ]; then
...
function install_dependencies {
    ...
    if [ "$PM" == "" ]; then
        ...
        sudo pacman -S --needed --noconfirm wget curl acl unzip tar rsyslog base-devel git
        
        # Enable rsyslog for traditional syslog support (optional on Arch)
        info "Enabling rsyslog service (Arch uses journald by default)"
        sudo systemctl enable rsyslog 2>/dev/null || info "rsyslog service config done"
        sudo systemctl start rsyslog 2>/dev/null || info "rsyslog service already running"
    else
    ...
    if sudo [ ! -e $SPLUNK_HOME/bin/splunk ]; then
        ...
        case "$PACKAGE" in
            auto )
                ...
                case "$PM" in
                    pacman )
                        info "Arch Linux detected: using .tgz package format"
                        PACKAGE="tgz"
                        install_splunk
                    ;;
                    apt-get )
...
function setup_forward_server {
    print_banner "Configuring Forward Server"
    
    info "Setting forward server to $1"
...
function setup_splunk {
    ...
    # Wait for Splunk to actually start before trying to login
    ...
    # FIXED: Added retry limit to prevent infinite loops
    res=-1
    attempt=0
    while [ $res -ne 0 ] && [ $attempt -lt $MAX_LOGIN_ATTEMPTS ]; do
        ...
        if [ $res -ne 0 ]; then
            ...
            if [ $attempt -lt $MAX_LOGIN_ATTEMPTS ]; then
                error "Login failed (attempt $attempt/$MAX_LOGIN_ATTEMPTS). Please verify credentials."
                SPLUNK_PASSWORD=""  # Reset password to prompt again
            else
                error "Failed to login after $MAX_LOGIN_ATTEMPTS attempts"
                error "Please verify Splunk is running: sudo systemctl status SplunkForwarder"
                error "You can manually login later with: sudo -H -u splunk $SPLUNK_HOME/bin/splunk login"
                exit 1
            fi
        fi
    done
...
function add_system_logs {
    ...
    # Arch-specific: Add systemd journal monitoring
    if [ "$PM" == "pacman" ]; then
        info "Adding systemd journal monitoring for Arch Linux"
        sudo -H -u splunk $SPLUNK_HOME/bin/splunk add monitor "/var/log/journal" -index "system" -sourcetype "systemd:journal" 2>/dev/null || info "Journal monitoring configured"
        # Add splunk user to systemd-journal group
        sudo usermod -aG systemd-journal splunk 2>/dev/null || info "User already in systemd-journal group"
    fi
...
function add_firewall_logs {
    ...
    # iptables logging configured in harden.sh
    add_monitor "/var/log/kern.log" "$INDEX"
    
    # Add pf logs for BSD systems
    add_monitor "/var/log/pflog" "$INDEX"
}

function add_web_logs {
    print_banner "Adding web server logs"
    INDEX="web"
...
function add_package_manager_logs {
    print_banner "Adding package manager logs"
    INDEX="system"
    ...
    # Arch-specific: pacman logs
    if [ "$PM" == "pacman" ]; then
        info "Adding pacman log monitoring for Arch Linux"
        add_monitor "/var/log/pacman.log" "$INDEX"
        # Set permissions for pacman log
        sudo setfacl -m g:splunk:r /var/log/pacman.log 2>/dev/null || info "ACL set for pacman.log"
    fi
}

function add_cron_logs {
    print_banner "Adding cron/scheduled task logs"
    INDEX="system"
...
function add_mail_logs {
    print_banner "Adding mail server logs"
    INDEX="system"
...
function install_auditd {
    ...
    if [ "$PM" == "" ]; then
        info "No package manager detected. Skipping auditd installation."
        return 1
    fi
    ...
    if [ "$PM" == "pacman" ]; then
        ...
        if ! pacman -Qi audit &>/dev/null; then
            info "Installing audit package"
            sudo pacman -S --needed --noconfirm audit
        else
            info "audit package already installed"
        fi
        
        # Enable and start auditd
        info "Enabling auditd service"
        sudo systemctl enable auditd 2>/dev/null || info "auditd service already enabled"
        sudo systemctl start auditd 2>/dev/null || info "auditd service already running"
        
        # Set permissions for audit log on Arch
        if sudo test -e /var/log/audit/audit.log; then
            info "Setting ACL permissions for audit.log on Arch"
            sudo setfacl -m g:splunk:r /var/log/audit/audit.log
            sudo setfacl -dm g:splunk:r /var/log/audit/
        fi
    else
        # Other distributions
        if sudo "$PM" list installed auditd &>/dev/null || sudo "$PM" list installed audit &>/dev/null; then
            info "auditd already installed"
        else
            sudo "$PM" install -y auditd || sudo "$PM" install -y audit
        fi
        ...
        if command -v systemctl &>/dev/null; then
            ...
            info "Starting auditd"
            sudo service auditd start
        fi
    fi
    
    if sudo [ -e /var/log/audit/audit.log ]; then
        info "auditd installed successfully"
        download "$GITHUB_URL/linux/audit/auditd.rules" auditd.rules
        if sudo [ -f /etc/audit/rules.d/audit.rules ]; then
            sudo mv auditd.rules /etc/audit/rules.d/audit.rules
        elif sudo [ -f /etc/audit/audit.rules ]; then
            sudo mv auditd.rules /etc/audit/audit.rules
        fi
        sudo systemctl restart auditd || sudo service auditd restart
        AUDITD_SUCCESSFUL=true
        return 0
    else
        error "auditd failed to install"
        return 1
    fi
}
...
function install_snoopy {
    version=$1
    print_banner "Installing Snoopy version $version"
    
    download "https://github.com/a2o/snoopy/releases/download/snoopy-$version/snoopy-$version.tar.gz" "snoopy-$version.tar.gz"
    if [ ! -f "snoopy-$version.tar.gz" ]; then
        error "Failed to download Snoopy"
        return 1
    fi
    
    tar -xzf "snoopy-$version.tar.gz"
    cd "snoopy-$version" || return 1
    
    # Build dependencies for Arch
    if [ "$PM" == "pacman" ]; then
        info "Installing build dependencies for Arch"
        sudo pacman -S --needed --noconfirm base-devel git socat
    fi
    
    # Snoopy with tests disabled
    if ./configure --enable-config-file --with-config-file=/etc/snoopy.ini --disable-datasource-tid --disable-datasource-sid && make && sudo make install; then
        cd ..
        if command -v snoopy-enable &>/dev/null; then
...
function main {
    ...
    echo "   Usage: ./splunk2.sh -a <LOG_PATH>"
    ...
    if [ "$PM" == "pacman" ]; then
        ...
        echo "   - Systemd journal is being monitored at /var/log/journal"
        echo "   - Pacman logs are being monitored at /var/log/pacman.log"
        echo "   - Audit log permissions have been set with ACLs"
        if [ "$SNOOPY_SUCCESSFUL" == true ]; then
            echo "   - Consider restarting to ensure Snoopy captures all processes"
        fi
    fi
    echo
}
