...
function download {
    ...
    if ($LOCAL_INSTALL -and $url.StartsWith($GITHUB_URL)) {
        ...
        try {
            ...
            error "Download failed with Invoke-WebRequest, trying WebClient..."
            try {
...
function detect_version {
    if ($arch -eq 64) {
        switch ($version) {
            ...
            default { error "Invalid version: $version"; exit 1 }
        }
    }
    elseif ($arch -eq 32) {
        switch ($version) {
...
function install_splunk {
    ...
    # Check for existing installer first
    $installer_path = "$pwd\splunk.msi"
    if (Test-Path "C:\splunkforwarder.msi") {
        print "Found existing installer at C:\splunkforwarder.msi"
        $installer_path = "C:\splunkforwarder.msi"
    } elseif (Test-Path "$pwd\splunkforwarder.msi") {
        print "Found existing installer at $pwd\splunkforwarder.msi"
        $installer_path = "$pwd\splunkforwarder.msi"
    } else {
...
function install_ossec {
    ...
    Move-Item -Path "$OSSECDIR\ossec.conf" "$OSSECDIR\ossec.conf.bak" -Force -ErrorAction SilentlyContinue
...
if ($ip -eq "indexer" -or $ip -eq "i") {
    $SPLUNKDIR = "C:\Program Files\Splunk"
