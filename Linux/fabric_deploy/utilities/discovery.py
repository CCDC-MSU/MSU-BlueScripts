...
from .ssh_runner import CommandResult, run_command, run_command_chain
...
class SystemDiscovery:
    ...
    def __init__(self, connection: Connection, credentials: ServerCredentials):
        ...
        self.server_info = ServerInfo(
            hostname=credentials.host, credentials=credentials
        )
        self._os_family = OSFamily.UNKNOWN.value
        self.user_manager = UserManager(
            available_commands=self.server_info.available_commands,
            groups=self.server_info.groups,
            sudoers_info=self.server_info.sudoers_info,
        )
    ...
    def discover_system(self) -> ServerInfo:
        ...
        discovery_tasks = [
            ("controller ip", self._discover_controller_ip),
        ...
        for task_name, task_func in discovery_tasks:
            try:
                ...
                logger.debug(
                    "Finished task: %-20s. %s",
                    task_name,
                    self._format_task_result(task_name),
                )
        ...
        logger.info(
            f"Discovery completed for {self.credentials.host}. Success: {self.server_info.discovery_successful}"
        )
    ...
    def _format_task_result(self, task_name: str) -> str:
        ...
        try:
            ...
            if task_name == "controller ip":
                return f"ip={self.server_info.controller_ip!r}"

            if task_name == "docker":
                return f"has_docker={self.server_info.has_docker}"
    ...
    def _run_command(
        self, command: str, warn: bool = True, timeout: int = 30
    ) -> CommandResult:
        """
        Run a command and return the result.
        Delegates to shared ssh_runner.run_command().
        """
        return run_command(self.conn, command, warn=warn, timeout=timeout)

    def _try_command_chain(
        self, commands: list[str], processor: Callable | None = None
    ) -> str | None:
        """
        ...
        """
        result = run_command_chain(self.conn, commands)
    ...
    def _discover_os_info(self):
        ...
        if result.success and result.output:
            ...
            os_info.version = os_release.get(
        ...
        for file_path, (family, name) in distro_files.items():
            ...
            if result.success and result.output == "exists":
                ...
                version_result = self._run_command(
                    f"cat {file_path} 2>/dev/null | head -1"
                )
    ...
    def _discover_init_system(self):
        ...
        if init_system in ["unknown", "init"]:
            ...
            # Check OpenRC
                ...
                self._run_command("test -d /run/openrc").success
                or self._run_command("command -v rc-status").success
            ):
                init_system = "openrc"
            ...
            # BSD checks
            elif self._os_family in [
                OSFamily.FREEBSD.value,
                OSFamily.OPENBSD.value,
                OSFamily.NETBSD.value,
                OSFamily.BSDGENERIC.value,
            ]:
                init_system = "bsd"
    ...
    def _test_valid_user(self, username) -> bool:
        ...
        result = self._run_command(
            f'su - {username} -c "true" 2>/dev/null', warn=True, timeout=2
        )
    ...
    def _discover_users(self):
        ...
        for user in users:
            ...
            key_check_result = self._run_command(
                f'test -f {auth_keys_path} && test -s {auth_keys_path} && echo "has_key"',
                warn=True,
            )
    ...
    def _discover_security_tools(self):
        ...
        tools_to_check = ["iptables", "auditctl", "rsyslogd"]
    ...
    def _discover_system_resources(self):
        ...
        cpu_count = self._try_command_chain(
            ["nproc", "sysctl -n hw.ncpu", "grep -c ^processor /proc/cpuinfo"]
        )
    ...
    def _discover_controller_ip(self):
        """Discover the IP address of the controller (hardening machine) as seen by this host.

        Uses the SSH connection's source IP to identify the machine running the hardening process.
        This is useful for:
        - Firewall rules to allow the controller
        - Audit log correlation
        - Debugging network issues
        """
        controller_ip = ""

        # Method 1: Use 'who -m' to get current login source
        # This shows the IP/hostname of the current SSH session
        result = self._run_command("who -m 2>/dev/null")
        if result.success and result.output:
            # Output format: "root     pts/0        2024-01-17 05:30 (192.168.1.100)"
            import re

            match = re.search(r"\(([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)\)", result.output)
            if match:
                controller_ip = match.group(1)

        # Method 2: Use ss to find the source IP of our SSH connection
        # Look for established SSH connections to my session
        if not controller_ip:
            result = self._run_command(
                "ss -tn state established '( sport = :22 )' 2>/dev/null | head -5"
            )
            if result.success and result.output:
                # Output format: "ESTAB 0 0 10.0.0.3:22 192.168.1.100:54321"
                # The peer address is in the last column
                import re

                for line in result.output.splitlines():
                    match = re.search(
                        r"([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+):\d+\s*$", line
                    )
                    if match:
                        controller_ip = match.group(1)
                        break

        # Method 3: Parse SSH_CLIENT or SSH_CONNECTION env var
        if not controller_ip:
            result = self._run_command("echo $SSH_CLIENT 2>/dev/null")
            if result.success and result.output:
                # SSH_CLIENT format: "192.168.1.100 54321 22"
                parts = result.output.split()
                if parts:
                    controller_ip = parts[0]

        self.server_info.controller_ip = controller_ip

    def _discover_docker(self):
        """Detect if Docker is installed on the server.

        TODO: Implement actual docker detection. For now, hardcoded for 10.5.1.6.
        """
        # Hardcoded for now - only 10.5.1.6 has docker
        self.server_info.has_docker = self.credentials.host == "10.5.1.6"
