...
- Per-host log files are created under `logs/{task}/{host}/{timestamp}.log` by `_host_log_handler` (`fabfile.py:94-110`).
- `run_script` uses a separate output layout under `script_outputs/<script_name>/` (`fabfile.py:518-527`).
...
- `hosts.txt` parsing is handled by `parse_hosts_file` from `utilities.utils` and drives `ServerCredentials` creation (`fabfile.py:179-181`, `fabfile.py:323-325`, `fabfile.py:752-754`).
