...
from utilities.modules.base import CommandAction, HardeningModule, PythonAction
...
class ModuleTester:
        ...
        try:
            ...
            for attr_name in dir(module):
                ...
                if (
                    ...
                    and issubclass(attr, HardeningModule)
                    and attr != HardeningModule
                ):
                    return attr
    ...
    def test_module(self, module_name, host_index=0, dry_run=True, verbose=False):
        ...
        try:
            with self._connect_to_host(server_creds) as conn:
                ...
                logger.info(
                    f"Discovery successful - OS: {server_info.os.distro} {server_info.os.version}"
                )
                ...
                if actions:
                    ...
                    with open(cmd_file, "w") as f:
                        f.write(
                            f"Actions for {module_name} module on {server_creds.host}\n"
                        )
                        f.write("=" * 60 + "\n\n")
                        for i, action in enumerate(actions, 1):
                            f.write(f"{i:2d}. {action.description}\n")
                            if isinstance(action, CommandAction):
                                f.write("    Type: Shell Command\n")
                            ...
                            if action.os_families:
                                f.write(
                                    f"    OS Families: {[f.value for f in action.os_families]}\n"
                                )
                            f.write("\n")

                    logger.info(
                        f"{len(actions)} actions ready - details written to: {cmd_file}"
                    )
                ...
                results = self._apply_all_with_verbose_logging(
                    module_instance, dry_run, verbose
                )
    ...
    def _apply_all_with_verbose_logging(self, module_instance, dry_run, verbose):
        ...
        if not module_instance.is_applicable():
            logger.info(
                f"Module {module_instance.get_name()} is not applicable to this system"
            )
            return []
        ...
        for i, action in enumerate(actions, 1):
            ...
            if verbose:
                logger.info(
                    f"[{i}/{len(actions)}] [{action_type}] Starting: {action.description}"
                )
                if isinstance(action, CommandAction):
            ...
            try:
                if dry_run:
                    if verbose:
                        logger.info(
                            f"[{i}/{len(actions)}] [DRY RUN] Would execute: {action.description}"
                        )
                    ...
                    if verbose:
                        logger.info(
                            f"[{i}/{len(actions)}] Applying: {action.description}"
                        )

                    # Add timeout handling for live execution
                    result = self._apply_action_with_timeout(
                        module_instance, action, timeout=300
                    )
                ...
                if verbose:
                    ...
                    logger.info(
                        f"[{i}/{len(actions)}] [{status}] Completed in {execution_time:.2f}s"
                    )
                ...
                error_msg = str(e)
                logger.error(
                    f"[{i}/{len(actions)}] EXCEPTION after {execution_time:.2f}s: {error_msg}"
                )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Host index from hosts.txt (default: 0)",
    )
    parser.add_argument(
        "--live", action="store_true", help="Run in live mode (default is dry-run)"
    )
    ...
    if not args.module:
        print(
            "No module specified. Use --list to see available modules or --module <name> to test."
        )
        tester.list_modules()
        return
    ...
    success = tester.test_module(
        args.module, args.host_index, dry_run, verbose=args.verbose
    )
