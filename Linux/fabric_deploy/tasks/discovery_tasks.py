...
def discover(c, host, user=None, key_file=None, password=None):
    ...
    try:
        ...
        if is_connection_reset(e):
            logger.error(
                f"Connection failed to {host}: Connection reset by peer (Firewall/Hostile Action)"
            )
        else:
            ...
            try:
                ...
                with Connection(
                    ...
                    if server_info.discovery_successful:
                        ...
                        logger.info(
                            f"OS: {server_info.os.distro} {server_info.os.version}"
                        )
                        logger.info(f"Users: {server_info.regular_usernames}")
                        logger.info(f"Services: {len(server_info.services)} running")
                        logger.info(
                            f"Security tools: {[k for k, v in server_info.security_tools.items() if v]}"
                        )
                ...
                if is_connection_reset(e):
                    logger.error(
                        f"Discovery failed for {server_creds.host}: Connection reset by peer"
                    )
                else:
                    logger.error(f"Discovery failed for {server_creds.host}: {e}")
                server_info = ServerInfo(
                    hostname=server_creds.host, credentials=server_creds
                )
