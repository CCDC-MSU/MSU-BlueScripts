...
def _upload_scripts_internal(conn, dry_run=False):
    """Internal helper to sync scripts dir"""
    local_scripts_path = Path(__file__).parent.parent / "scripts"

    if not local_scripts_path.exists():
        logger.warning(
            f"No local 'scripts' directory found to upload at {local_scripts_path}"
        )
        return

    remote_scripts_path = "/root/hardening-scripts"

    if dry_run:
        logger.info(f"[DRY RUN] Would upload scripts from {local_scripts_path} to {remote_scripts_path}")
        return

    logger.info("Uploading scripts directory...")
    conn.run(f"mkdir -p {remote_scripts_path}")

    # Create tar in memory
    fh = BytesIO()
    with tarfile.open(fileobj=fh, mode="w:gz") as tar:
        tar.add(local_scripts_path, arcname=".")
    tar_data = fh.getvalue()

    # Upload tar
    tmp_tar = "/tmp/ccdc_scripts_archive.tar.gz"
    conn.put(BytesIO(tar_data), tmp_tar)

    # Extract
    conn.run(f"tar -xzf {tmp_tar} -C {remote_scripts_path}")
    conn.run(f"rm {tmp_tar}")
    logger.info(f"Scripts uploaded to {remote_scripts_path}")


@task
def upload_hardening_scripts(c, hosts_file="hosts.txt", dry_run=False):
    """Upload scripts directory to remote servers"""
    _configure_parallel_logging()
    ...
    def _upload_host(server_creds):
        ...
        with _host_log_handler("scripts-upload", host_id) as _:
            try:
                ...
                if is_connection_reset(e):
                    logger.error(
                        f"Scripts upload failed for {server_creds.host}: Connection reset by peer"
                    )
                else:
                    logger.error(f"Scripts upload failed: {e}")
...
def _write_runbash_output(
    output_file, server_creds, script_path, steps, exec_result, error_message=None
):
    """Write per-host script output to a local file."""
    try:
        with open(output_file, "w") as f:
            host_header = (
                f"{server_creds.display_name} ({server_creds.host})"
                if server_creds.friendly_name
                else server_creds.host
            )
...
def run_script(
    ...
    def _run_on_host(server_creds):
        ...
        try:
            ...
            config_overrides = {"sudo": {"password": None}, "load_ssh_configs": False}
            ...
            with Connection(
                ...
                create_result = _run_remote(
                    conn, f"mkdir -p {remote_dir} && chmod 755 {remote_dir}"
                )
                steps.append(("create staging dir", create_result))
                if not create_result.ok:
                    logger.error(
                        f"{server_creds.display_name}: failed to create staging dir"
                    )
                    _write_runbash_output(
                        output_file, server_creds, script_path, steps, None
                    )
                    return {
                        "host": server_creds.host,
                ...
                logger.info(
                    f"{server_creds.display_name}: executing script with nohup (remote log: {remote_log})"
                )
                ...
                if exec_result.ok:
                    logger.info(
                        f"{server_creds.display_name}: script completed successfully (exit code: {exec_result.exited})"
                    )
                else:
                    logger.warning(
                        f"{server_creds.display_name}: script exited with code {exec_result.exited}, check remote log: {remote_log}"
                    )
                ...
                _write_runbash_output(
                    output_file, server_creds, script_path, steps, exec_result
                )

                # Determine status based on exit code
                if not exec_result.ok:
                    logger.error(
                        f"{server_creds.display_name}: script failed with exit code {exec_result.exited}"
                    )
                    status = "failed"
                else:
                    status = "ok"
            ...
            if is_connection_reset(e):
                logger.error(
                    f"{server_creds.display_name}: Connection reset by peer (Firewall/Hostile Action)"
                )
            else:
                logger.error(f"{server_creds.display_name}: {e}")
            _write_runbash_output(
                output_file, server_creds, script_path, [], None, error_message=str(e)
            )
            return {
                "host": server_creds.host,
                "exit_code": None,
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_map = {
            executor.submit(_run_on_host, server): server for server in servers
        }
    ...
    for item in sorted(summary, key=lambda x: x["host"]):
        display = next(
            (s.display_name for s in servers if s.host == item["host"]), item["host"]
        )
    ...
    logger.info(
        f"Completed in {duration} | ok={successful_runs} failed={failed_runs} dry-run={dry_runs}"
    )
