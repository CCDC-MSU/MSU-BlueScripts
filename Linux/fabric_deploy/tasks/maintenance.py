...
from utilities.utils import is_connection_reset, parse_hosts_file
...
def reset_ssh(c, hosts_file="hosts.txt", restart=True):
    ...
    def _reset_host(server_creds):
        ...
        with _host_log_handler(
            "reset-ssh", host_id, datetime.now().strftime("%Y%m%d_%H%M%S")
        ) as _:
            try:
                # Connection setup
                connect_kwargs = {
                ...
                with Connection(
                    ...
                    res = conn.run(
                        "ls -t /etc/ssh/sshd_config.fabric.backup* 2>/dev/null | head -1",
                        warn=True,
                    )
                    ...
                    # 2. Restore backup
                    conn.run(f"cp {backup_file} /etc/ssh/sshd_config")
                    logger.info("Restored backup to /etc/ssh/sshd_config")

                    # 3. Restart Service
                    if restart:
                        logger.info("Restarting SSH service...")
                        # Use ServiceManager with unknown init system to get fallback chain
                        svc_mgr = ServiceManager(
                            init_system="unknown", available_commands=[]
                        )
                        # Try sshd first (most common), then ssh
                        restart_cmd = (
                            f"({svc_mgr.restart('sshd')}) || ({svc_mgr.restart('ssh')})"
                        )
                        res = conn.run(restart_cmd, warn=True)
                        if res.ok:
                            logger.info("SSH service restarted successfully")
                        else:
                            host_identifier = (
                                server_creds.friendly_name
                                if server_creds.friendly_name
                                else server_creds.host
                            )
                            logger.error(
                                f"Failed to restart SSH on host {host_identifier}: {res.stderr}"
                            )
                            return {
                ...
                if is_connection_reset(e):
                    logger.error(
                        f"Reset failed for {server_creds.host}: Connection reset by peer"
                    )
                else:
