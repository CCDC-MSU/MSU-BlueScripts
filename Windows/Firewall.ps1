...
if ($Argsarray.Count -lt 3) {
    Write-Host 'Usage - .\Firewall.ps1 "<Remote Management Subnet>;<Network Subnet>;NOTNATS"'
    break
}
...
if (-not (Get-Command -Name New-NetFirewallRule -ErrorAction SilentlyContinue) -or -not (Get-Command Get-ScheduledTask)) {
    ...
    if ($Notnats -ne "NOTNATS") {
        netsh advfirewall firewall add rule name="NotNats Inbound" dir=in protocol=any remoteip=$Notnats action=allow
        netsh advfirewall firewall add rule name="NotNats Outbound" dir=out protocol=any remoteip=$Notnats action=allow
    }
    ...
    if ($Notnats -ne "NOTNATS") {
        New-NetFirewallRule -DisplayName "NotNats Inbound" -Direction Inbound -Protocol Any -RemoteAddress $Notnats
        New-NetFirewallRule -DisplayName "NotNats Outbound" -Direction Outbound -Protocol Any -RemoteAddress $Notnats
    }
